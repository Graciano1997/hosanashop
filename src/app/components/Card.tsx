...
export default function ProductCard({ product, extraStyle }: ProductCardProps) {
  ...
  return (
    <>
      <div className={`bg-white/50  shadow hover:shadow-lg transition overflow-hidden p-4 ${extraStyle}`}>
        ...
        <div className="p-4 flex flex-col">
          ...
          <DefaultButton isAdded={isAdded} text="Adicionar" extraStyle={""} actionHandler={() => {
            if (!isAdded) { addItem(product, productDefaultAmount) };
            setIsAdded(true);
          }} />
        </div>
      </div>
      {visible &&
        <div className="absolute">
          <Modal closeHandler={() => { setVisible(false) }}>
            <div className="h-full md:flex md:justify-center">
