...
<div className="fixed right-4 bottom-6 p-3 flex items-center" style={{ zIndex: 1000 }}>
    ...
    <Modal closeHandler={()=>{setCheckoutVisibility(false)}}>
      ...
      <div className="p-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 shadow p-2 items-center">
          ...
          <button onClick={()=>{removeItem(product.item)}}  className="bg-red-300 p-2 text-white">Remover</button>
      </div>
        )}
        </div>
  ...
  <DefaultButton text="Continuar comprando" extraStyle={'p-5'} actionHandler={()=>setCheckoutVisibility(false)} />
</div>
